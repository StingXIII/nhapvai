...
const FandomGenesisScreen: React.FC<FandomGenesisScreenProps> = ({ onBack }) => {
  ...
  const handleGenerateSelectedArcs = async () => {
    ...
    setLoadingStates(p => ({...p, arc: true}));
    const arcsToProcess: string[] = Array.from(selectedArcs); 
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        // FIX: Replaced for...of with a standard for loop to iterate over FileList for better compatibility.
  ...
  const handleTrainFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        // FIX: Replaced for...of with a standard for loop to iterate over FileList, resolving potential 'unknown' type issues.
        for (let i = 0; i < files.length; i++) {
            const file = files[i];
