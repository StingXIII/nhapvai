...
const cleanNarration = (text: string): string => {
    ...
    // 1. Remove XML tags leftovers
    clean = clean.replace(/<\/?thinking>/gi, '');
    clean = clean.replace(/<\/?world_sim>/gi, '');

    // 2. Remove AI Explanations/Checklists (The "Leak" Fix)
    // Matches patterns like "[: Yes.", "Confidence Score: 5/5", "Strategizing complete"
    clean = clean.replace(/^\[:\s*(Yes|No)\.?.*$/gmi, ''); 
    clean = clean.replace(/^Confidence Score:.*$/gmi, '');
    clean = clean.replace(/^Strategizing complete.*$/gmi, '');
    
    // Remove "Protocol Application" blocks that sometimes leak
    clean = clean.replace(/^\*\*Command Reflex Protocol Application:\*\*[\s\S]*?(?=\n\n|\n[A-Z]|$)/gmi, '');
    clean = clean.replace(/^\*\*Deep Simulation & Narrative Plan:\*\*[\s\S]*?(?=\n\n|\n[A-Z]|$)/gmi, '');
    
    // Remove bullet points that look like system explanations (e.g., "* **Condition...:**")
    clean = clean.replace(/^\s*\*\s*\*\*.*?:.*/gm, ''); 
    clean = clean.replace(/^\s*\*\s*Điều kiện kích hoạt.*/gmi, '');
    clean = clean.replace(/^\s*\*\s*Sự kiện.*/gmi, '');

    // 3. Remove internal headers
    clean = clean.replace(/\[TIN TỨC THẾ GIỚI\]/gi, '');

    // 4. Remove leading/trailing horizontal rules or excessive whitespace
...
export const startGame = async (config: WorldConfig): Promise<{ narration: string; tags: ParsedTag[]; worldSim?: string; thinking?: string }> => {
    ...
    let finalNarration = cleanNarration(parsed.narration);

    // NEW FALLBACK LOGIC for startGame
    if (!finalNarration) {
        if (parsed.worldSim) {
            finalNarration = `[Thế giới đã được kiến tạo. Các sự kiện đầu tiên bắt đầu diễn ra...]\n\n----------------\n\n${cleanNarration(parsed.worldSim)}`;
        } else {
            finalNarration = `[Thế giới đã được kiến tạo. Cuộc phiêu lưu của bạn sắp bắt đầu.]`;
        }
    } else {
        if (parsed.worldSim) {
...
export const getNextTurn = async (gameState: GameState, codeExtractedTime?: TimePassed): Promise<{ narration: string; tags: ParsedTag[]; worldSim?: string; thinking?: string; newSummary?: string }> => {
    ...
    // NEW FALLBACK LOGIC
    if (!finalNarration) {
        // Nếu narration trống sau khi làm sạch, tạo một thông báo dự phòng.
        if (parsed.worldSim) {
            // Nếu có world_sim, sử dụng nó làm ngữ cảnh.
            finalNarration = `[Hành động của bạn không tạo ra kết quả trực tiếp, nhưng ở một nơi khác, các sự kiện sau đã diễn ra...]\n\n----------------\n\n${cleanNarration(parsed.worldSim)}`;
        } else {
            // Nếu cả hai đều trống, cung cấp một thông báo lỗi/phản hồi rõ ràng.
            finalNarration = `[AI không thể tạo ra phản hồi cho hành động này. Có thể hành động của bạn không có tác động đáng kể, hoặc đã có lỗi xảy ra. Vui lòng thử một hành động khác.]`;
        }
    } else {
        // Nếu narration TỒN TẠI, nối thêm world_sim nếu nó cũng tồn tại.
        if (parsed.worldSim) {
            finalNarration = `${finalNarration}\n\n----------------\n\n${cleanNarration(parsed.worldSim)}`;
        }
    }
