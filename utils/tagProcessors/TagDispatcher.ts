...
import { processFactionUpdate, processEntityDiscovered, processEntityDefinition } from './EntityProcessor';
...
import { processBeginCombat } from './CombatProcessor';
import { processWifeAffinityUpdate, processSlaveAffinityUpdate, processPrisonerAffinityUpdate } from './affinityTagProcessors';
...
export function dispatchTags(currentState: GameState, tags: ParsedTag[]): { finalState: GameState, vectorUpdates: VectorUpdate[] } {
    ...
    const result = validatedTags.reduce(
        (acc: { state: GameState; updates: VectorUpdate[] }, tag) => {
            ...
            switch (tag.tagName) {
                case 'BEGIN_COMBAT':
                    processResult = processBeginCombat(acc.state, tag.params);
                    break;
                ...
                case 'NPC_UPDATE':
                    ...
                    break;
                case 'WIFE_UPDATE':
                    processResult = processWifeAffinityUpdate(acc.state, tag.params);
                    break;
                case 'SLAVE_UPDATE':
                    processResult = processSlaveAffinityUpdate(acc.state, tag.params);
                    break;
                case 'PRISONER_UPDATE':
                    processResult = processPrisonerAffinityUpdate(acc.state, tag.params);
                    break;
                ...
                case 'LORE_DISCOVERED':
                    ...
                    break;
                case 'ENTITY_DEFINITION': // Thẻ mới cho Hybrid Lookup
                    processResult = processEntityDefinition(acc.state, tag.params);
                    break;
                case 'MEMORY_ADD':
