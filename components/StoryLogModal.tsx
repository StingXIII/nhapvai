...
const StoryLogModal: React.FC<StoryLogModalProps> = ({ isOpen, onClose, history, title, initialScrollTop = null }) => {
  ...
  // Xử lý sự kiện cuộn để hiện nút cuộn xuống
  const handleScroll = () => {
    if (scrollRef.current) {
        const { scrollTop, scrollHeight, clientHeight } = scrollRef.current;
        // Hiện nút nếu người dùng cuộn lên trên khoảng 100px so với đáy
      ...
      >
        ...
        <div className="relative flex-grow min-h-0 overflow-hidden">
            <div 
                ...
                className="h-full overflow-y-auto pr-3 space-y-6"
            >
            ...
            <button
                ...
                className={`absolute bottom-4 right-6 p-2 bg-slate-700/80 hover:bg-slate-600 text-white rounded-full shadow-lg transition-all duration-300 z-20 transform ${showScrollButton ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}
