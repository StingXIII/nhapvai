...
const AffinityDisplay: React.FC<{ affinity: number }> = ({ affinity }) => {
...
const EntityInfoModal: React.FC<EntityInfoModalProps> = ({ isOpen, onClose, title, description, type, details, questData, isLoading, affinity }) => {
  
  // Logic parse stats JSON
  const parsedStats = useMemo(() => {
    if (!details?.stats) return null;
    try {
        const trimmed = details.stats.trim();
        // Chỉ cố gắng parse nếu nó bắt đầu bằng dấu ngoặc nhọn (dấu hiệu của JSON object)
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] p-4" onClick={onClose}>
      ...
      >
        ...
        {typeof affinity === 'number' && !isLoading && (
            <div className="mb-4">
                <AffinityDisplay affinity={affinity} />
            </div>
        )}

        <div className="max-h-[60vh] overflow-y-auto pr-2 space-y-4 min-h-[100px] flex flex-col custom-scrollbar">
            {isLoading ? (
                <div className="flex-grow flex items-center justify-center py-10">
                    <Spinner text="Đang truy xuất kiến thức thần thông..." />
                </div>
            ) : isQuest && questData ? (
                <>
                    {/* 1. Mục tiêu hiện tại */}
                ...
                <>
                    ...
                    {details && (
                        <div className="mt-4 border-t border-slate-700 pt-4 text-sm space-y-4">
                            
                            {/* Stats Section: Render Visual if parsed, else Text */}
                            {parsedStats ? (
                                <div className="bg-slate-900/30 p-3 rounded-lg border border-slate-700/50">
                                    <StatsDisplay stats={parsedStats} title="Chỉ Số Chi Tiết" />
                                </div>
                            ) : (
                                details.stats && (
                                    <div>
