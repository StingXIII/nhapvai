...
import StatsDisplay from './common/StatsDisplay'; // Import StatsDisplay
...
const AffinityDisplay: React.FC<{ affinity: number, size?: 'small' | 'large' }> = ({ affinity, size = 'small' }) => {
    const color = affinity > 50 ? 'text-green-400' : affinity > 0 ? 'text-lime-400' : affinity < -50 ? 'text-red-500' : affinity < 0 ? 'text-orange-400' : 'text-slate-400';
    const sign = affinity > 0 ? '+' : '';

    if (size === 'large') {
        return (
            <div className={`inline-flex items-center gap-1.5 font-bold ${color} bg-slate-900/50 px-3 py-1.5 rounded-lg border border-slate-700`}>
                <Icon name="heart" className="w-5 h-5" />
                <span className="text-lg">{sign}{affinity}</span>
            </div>
        );
    }
    
    return (
        <span className={`inline-flex items-center gap-1 font-semibold ${color} text-xs`}>
            <Icon name="heart" className="w-3 h-3" />
            <span>{sign}{affinity}</span>
        </span>
    );
};
...
const TabButton: React.FC<{ active: boolean; onClick: () => void; children: React.ReactNode; iconName: any; }> = ({ active, onClick, children, iconName }) => (
    <button
        onClick={onClick}
        className={`flex items-center justify-start gap-3 px-3 py-3 text-sm font-semibold transition-colors duration-200 focus:outline-none w-full text-left rounded-md ${
            active
    ...
    >
        ...
        <Icon name="arrowDown" className="w-4 h-4 ml-auto transform -rotate-90 text-slate-500" />
    </button>
);
...
export const EncyclopediaModal: React.FC<EncyclopediaModalProps> = ({ isOpen, onClose, gameState, setGameState, onDeleteEntity }) => {
    ...
    // --- State cho Giao diện Mobile ---
    const [mobileView, setMobileView] = useState<'categories' | 'list' | 'details'>('categories');
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setCodexCommand('');
            setMobileView('categories'); // Reset mobile view
        }
    }, [isOpen]);
    ...
    useEffect(() => {
         ...
         setIsEditing(false);
         setSearchTerm('');
         if (mobileView !== 'categories') setMobileView('categories');
    }, [mainView]);
    ...
    const handleSelectItem = (item: AllEntities) => {
        setActiveItem(item);
        setIsEditing(false);
        setMobileView('details'); // Chuyển sang màn hình chi tiết
    };

    const handleSelectCategory = (key: string) => {
        setActiveTab(key);
        setMobileView('list'); // Chuyển sang màn hình danh sách
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            ...
            >
                ...
                {mainView === 'browse' && (
                    <div className="flex-grow overflow-hidden relative lg:grid lg:grid-cols-12">
                        {/* Panel 1: Danh sách Mục lục (Categories) */}
                        <div className={`
                            absolute inset-0 lg:relative lg:col-span-3 xl:col-span-2
                            transition-transform duration-300 ease-in-out
                            bg-slate-800/50 p-3 flex flex-col
                            ${mobileView === 'categories' ? 'translate-x-0' : '-translate-x-full'}
                            lg:translate-x-0
                            ...
                            <div className="flex-grow overflow-y-auto pr-2">
                                <div className="space-y-1">
                                    {fixedTabsConfig.map(tab => (
                                        ...
                                        <TabButton key={tab.key} active={activeTab === tab.key} onClick={() => handleSelectCategory(tab.key)} iconName={tab.icon}>{tab.label}</TabButton>
                                    ))}
                                    {dynamicCategories.length > 0 && <hr className="border-slate-700 my-3" />}
                                    {dynamicCategories.map(cat => (
                                        <TabButton key={cat} active={activeTab === cat} onClick={() => handleSelectCategory(cat)} iconName="hub">{cat}</TabButton>
                                    ))}
                                </div>
                            </div>
                        </div>

                        {/* Panel 2: Danh sách Thực thể (Item List) */}
                        <div className={`
                            absolute inset-0 lg:relative lg:col-span-4 xl:col-span-3
                            transition-transform duration-300 ease-in-out
                            border-r border-slate-700 flex flex-col bg-slate-800
                            ${mobileView === 'list' ? 'translate-x-0' : 'translate-x-full'}
                            ${mobileView === 'details' ? '-translate-x-full' : ''}
                            lg:translate-x-0
                        `}>
                             <div className="p-3 border-b border-slate-700 flex-shrink-0 flex items-center gap-2">
                                <button onClick={() => setMobileView('categories')} className="p-2 -ml-2 text-slate-400 hover:text-white lg:hidden">
                                ...
                                {filteredList.length > 0 ? (
                                    <ul className="p-2 space-y-1">
                                        {filteredList.map((item, index) => (
                                            <li key={index}>
                                                <button onClick={() => handleSelectItem(item)} className={`w-full text-left p-3 rounded-md transition-colors ${activeItem?.name === item.name ? 'bg-purple-600/30' : 'hover:bg-slate-700/50 bg-slate-900/40'}`}>
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <div className="flex items-center gap-2 flex-shrink-0">
                                                            {typeof (item as any).affinity === 'number' && (
                                                                <AffinityDisplay affinity={(item as any).affinity} />
                                                            )}
                        ...
                        {/* Panel 3: Chi tiết (Details) */}
                        <div className={`
                            absolute inset-0 lg:relative lg:col-span-5 xl:col-span-7
                            transition-transform duration-300 ease-in-out
                            overflow-y-auto bg-slate-900/50
                           ...
                           <div className="p-6">
                            {activeItem && !isEditing ? (
                                <div>
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-2 flex-grow min-w-0">
                                            <button onClick={() => setMobileView('list')} className="p-2 -ml-2 text-slate-400 hover:text-white lg:hidden flex-shrink-0">
                                                <Icon name="back" className="w-5 h-5"/>
                                            </button>
                                            <h3 className="text-2xl font-bold text-purple-300 truncate">{activeItem.name}</h3>
                                        </div>
                                        {activeTab !== 'knowledge' && (
                                    ...
                                    {typeof (activeItem as any).affinity === 'number' && (
                                        <div className="mb-4">
                                            <strong className="text-slate-400 block mb-1">Độ Thiện Cảm:</strong>
                                            <AffinityDisplay affinity={(activeItem as any).affinity} size="large" />
                                        </div>
                                    )}
                                    ...
                                    {gameState.worldConfig.enableStatsSystem && activeItem && 'stats' in activeItem && activeItem.stats && (
                                        <div className="my-6">
                                             <Accordion 
                                                title="Chỉ Số" 
                                                icon={<Icon name="info" />}
                                                ...
                                                startOpen={true}
                                             >
                                                <StatsDisplay stats={activeItem.stats} />
                                             </Accordion>
                                        </div>
                                    )}
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        if (targetList === 'encounteredNPCs') {
                                                            ...
                                                            handleFormChange('type', 'Nhiệm vụ');
                                                            handleFormChange('status', 'đang tiến hành');
                                                            handleFormChange('customCategory', '');
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-slate-500 text-center">
                                    ...
                                    <p className="text-lg">Chọn một mục lục và một vật phẩm để xem chi tiết</p>
                                </div>
                            )}
                            </div>
                        </div>
                ...
                {mainView === 'manage' && (
                     <div className="flex-grow p-6 overflow-y-auto">
                        ...
                        <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                            ...
                            <Button onClick={initiateAiOptimize} disabled={isAiOptimizing} variant="special" className="!w-auto !py-2 !px-4 !text-sm">
