...
type Tab = 'status' | 'quests';

const StatusHubModal: React.FC<StatusHubModalProps> = ({ 
    isOpen, onClose, statuses, quests, 
    onSelectStatus, onDeleteStatus, onSelectQuest, onDeleteQuest 
}) => {
  ...
  const tabButtonClasses = (isActive: boolean, color: string) => 
    `w-1/2 py-3 text-sm font-bold transition-colors duration-200 focus:outline-none border-b-2 ${
      isActive
        ? `text-${color}-300 border-${color}-400 bg-${color}-400/10`
