...
const POWER_SCALING_GUIDELINES = `
--- CẨM NANG THIẾT LẬP CẢNH GIỚI (POWER SCALING REFERENCE) ---
Bạn PHẢI phân loại thể loại của thế giới và áp dụng bộ khung cảnh giới tương ứng vào trường 'cultivationSystem':

1. **TU TIÊN (Cổ trang/Huyền huyễn phương Đông):**
   - Major Realms: "Luyện Khí - Trúc Cơ - Kim Đan - Nguyên Anh - Hóa Thần - Luyện Hư - Hợp Thể - Đại Thừa - Độ Kiếp"
   - Minor Realms: "1 Tầng - 9 Tầng" (Hoặc "Sơ kỳ - Trung kỳ - Hậu kỳ - Viên mãn")

2. **VÕ HIỆP (Kiếm hiệp truyền thống):**
   - Major Realms: "Bất Nhập Lưu - Tam Lưu - Nhị Lưu - Nhất Lưu - Tuyệt Đỉnh - Tông Sư - Đại Tông Sư"
   - Minor Realms: "Sơ Thành - Đại Thành" (Hoặc "Cấp 1 - Cấp 10")

3. **MA PHÁP (Western Fantasy):**
   - Major Realms: "Ma Pháp Học Đồ - Pháp Sư Cấp 1 -> Cấp 9 - Đại Pháp Sư - Hiền Giả - Ma Thần"
   - Minor Realms: "Giai đoạn 1 - Giai đoạn 5" (Hoặc "1 Sao - 5 Sao")

4. **THỢ SĂN / HỆ THỐNG (Modern Hunter/System):**
   - Major Realms: "Rank F - Rank E - Rank D - Rank C - Rank B - Rank A - Rank S - Rank SS - Rank SSS"
   - Minor Realms: "Cấp 1 - Cấp 10"

5. **KHOA HỌC VIỄN TƯỞNG (Sci-fi/Cyberpunk):**
   - Major Realms: "Cấp độ Cường hóa 1 - Cấp độ Cường hóa 10" (Hoặc "Thế hệ Chip 1.0 - 5.0")
   - Minor Realms: "Độ đồng bộ 10% - 100%"

6. **ĐỒNG NHÂN (Fanfiction):**
   - Bạn BẮT BUỘC phải tra cứu hệ thống gốc trong 'KIẾN THỨC NỀN' (VD: Naruto dùng Genin-Chunin-Jonin). Nếu không có kiến thức nền, hãy dùng bộ khung tương tự thể loại gần nhất.
`;
...
const getWorldCreationSchema = (generateMilestones: boolean) => {
    ...
    const entitySchema = {
        ...
        properties: {
            ...
            initialRealm: { type: Type.STRING, description: "Cảnh giới sức mạnh khởi đầu (Chỉ dành cho NPC). Phải khớp với một giá trị trong hệ thống tu luyện mà bạn thiết lập." },
        },
    ...
    const characterProperties: any = {
        ...
        initialRealm: { type: Type.STRING, description: "Cảnh giới khởi đầu của nhân vật chính. Phải khớp với hệ thống tu luyện." }
    };
    ...
    return {
        ...
        properties: {
            ...
            cultivationSystem: {
                type: Type.OBJECT,
                description: "Hệ thống cấp bậc tu luyện của thế giới. BẮT BUỘC PHẢI THIẾT KẾ.",
                properties: {
                    majorRealms: { type: Type.STRING, description: "Danh sách các Đại Cảnh Giới, phân cách bằng dấu gạch ngang (-). Bắt buộc phải phù hợp với thể loại (Tu tiên: Luyện Khí..., Võ hiệp: Tam Lưu..., Ma pháp: Cấp 1...)." },
                    minorRealms: { type: Type.STRING, description: "Danh sách các Tiểu Cảnh Giới. Nên có 10 cấp độ. VD: '1 Tầng - 9 Tầng' hoặc 'Sơ kỳ - ... - Viên Mãn'." }
                },
                required: ['majorRealms', 'minorRealms']
            },
            initialEntities: {
                type: Type.ARRAY,
                description: "Danh sách từ 5 đến 8 thực thể ban đầu (NPC, địa điểm, phe phái...). PHẢI đa dạng, không được chỉ có NPC.",
                items: entitySchema
            }
        },
        required: ['storyContext', 'character', 'difficulty', 'allowAdultContent', 'enableStatsSystem', 'enableMilestoneSystem', 'initialEntities', 'cultivationSystem']
    };
};
...
3.  **TÍNH CÁCH TÙY CHỈNH (CÔ ĐỌNG):** Trong đối tượng \`character\`, BẮT BUỘC đặt trường \`personality\` thành giá trị chuỗi 'Tuỳ chỉnh'. Trường \`customPersonality\` BẮT BUỘC phải là một danh sách các tính từ ngắn gọn, súc tích (VD: "Lạnh lùng, quyết đoán, thông minh tuyệt đỉnh"), TUYỆT ĐỐI KHÔNG viết thành văn xuôi dài dòng.
...
5.  **CHẤT LƯỢNG CAO:** Hãy tạo ra một thế giới phong phú. Bối cảnh phải cực kỳ chi tiết. Nhân vật phải có chiều sâu. 
6.  **ĐA DẠNG HÓA THỰC THỂ (\`initialEntities\`):** 
    a. Tạo ra 5 đến 8 thực thể ban đầu.
    b. **QUAN TRỌNG:** Danh sách này PHẢI ĐA DẠNG loại hình. BẮT BUỘC phải bao gồm: ít nhất 1 Địa điểm quan trọng (nơi bắt đầu), 1 Phe phái/Thế lực, 1 Vật phẩm (bảo vật/thư từ...), và các NPC. Đừng chỉ tạo toàn NPC.
    c. Với các NPC, nếu thế giới có hệ thống tu luyện, bạn BẮT BUỘC phải gán \`initialRealm\` (Cảnh giới) cho họ, và giá trị này phải thuộc hệ thống \`cultivationSystem\` bạn thiết lập.
...
9.  **HỆ THỐNG TU LUYỆN (QUAN TRỌNG):**
    a. BẮT BUỘC phải thiết kế một hệ thống tu luyện (\`cultivationSystem\`) DỰA TRÊN CẨM NĂNG CẢNH GIỚI ĐÃ CUNG CẤP ở trên.
    b. Hãy chọn đúng thể loại (Tu tiên, Võ hiệp, Ma pháp...) và điền chính xác danh sách Đại cảnh giới vào \`majorRealms\`.
    c. Đối với \`minorRealms\` (tiểu cảnh giới), hãy ưu tiên tạo ra một danh sách 10 cấp độ, ví dụ: "1 Tầng - 9 Tầng" hoặc "Sơ kỳ - ... - Viên Mãn".
10. **KHÔNG TẠO LUẬT:** Không tạo ra luật lệ cốt lõi (coreRules) hoặc luật tạm thời (temporaryRules).
11. **KHÔNG SỬ DỤNG TAG HTML:** TUYỆT ĐỐI không sử dụng các thẻ định dạng như <entity> hoặc <important> trong bất kỳ trường nào của JSON output.`;
...
${milestoneInstruction}
9.  **HỆ THỐNG TU LUYỆN (QUAN TRỌNG):**
    a. BẮT BUỘC phải thiết kế một hệ thống tu luyện (\`cultivationSystem\`) DỰA TRÊN CẨM NĂNG CẢNH GIỚI ĐÃ CUNG CẤP và DỮ LIỆU TÁC PHẨM GỐC.
    b. Hãy xác định đúng hệ thống cấp bậc của truyện (VD: Naruto: Genin-Chunin-Jonin; Đấu Phá: Đấu Giả-Đấu Sư...). Điền vào \`majorRealms\`.
    c. Đối với \`minorRealms\` (tiểu cảnh giới), hãy ưu tiên tạo ra một danh sách 10 cấp độ, ví dụ: "1 Tầng - 9 Tầng" hoặc "Sơ kỳ - ... - Viên Mãn".
10. **KHÔNG TẠO LUẬT:** Không tạo ra luật lệ cốt lõi (coreRules) hoặc luật tạm thời (temporaryRules).
11. **KHÔNG SỬ DỤNG TAG HTML:** TUYỆT ĐỐI không sử dụng các thẻ định dạng như <entity> hoặc <important> trong bất kỳ trường nào của JSON output.`;
